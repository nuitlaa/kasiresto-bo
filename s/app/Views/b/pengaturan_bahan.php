<?php $db = db_connect();
    ...
    $whr['a.status'] 		= 'active';
    $data = $db->table('product_component a')->join('account_store b','b.id=a.store')->select('a.*,b.name namatoko')->where($whr)->get()->getResultArray();
?>
...
<div class="card pt-4 mb-6 mb-xl-9"> 
	<div class="card-header border-0">
		...
		<div class="card-title">
			<h2 class="fw-bolder mb-0">Bahan</h2>
		</div> 
		...
		<div class="card-toolbar">
			...
			<a href="<?=site_url('pengaturan/bahan/tambah')?>" class="btn btn-sm btn-flex btn-light-primary">
	...
	<div id="kt_customer_view_payment_method" class="card-body pt-0">
		<table class="table">
			...
			<tr><th>Bahan</th><th>Deskripsi</th><th>Default Expired</th><th>Toko</th><th>Aksi</th></tr>
			<?php 
			$star = icon('star');
			$edit = icon('pencil2');
			$more = icon('more'); 
			...
			foreach($data as $K=>$v){  
				?>
					<tr id="row<?=$v['id']?>">
						...
						<td style="text-align: center;">
							<span class="fw-bold text-muted text-end me-3"><?=$v['expired']?> Hari</span>
						</td>
						<td style="text-align: center;">
							<span class="fw-bold text-muted text-end me-3"><?=$v['namatoko']?></span>
						</td>
						<td style="text-align: center;"> 
							<a href="<?=site_url('pengaturan/bahan/'.$v['id'])?>" class="btn btn-icon btn-active-light-primary w-30px h-30px me-3" ><?=$edit?></a>
							<a href="javascript:;" class="btn btn-icon btn-active-light-danger w-30px h-30px" onclick="removing('product_component',<?=$v['id']?>,'Bahan')"><?=$trash?></a>
						</td>
					</tr> 
			<?php } ?>
		</table>
