...
<div class="card pt-4 mb-6 mb-xl-9"> 
	...
	<div id="kt_customer_view_payment_method" class="card-body pt-0">
		...
		foreach($produk as $K=>$v){ 
			...
			?>
			<div class="py-0" data-kt-customer-payment-method="row" id="row<?=$v['id']?>">
				<div class="py-3 d-flex flex-stack flex-wrap">
					...
					<div class="d-flex align-items-center flex-wrap gap-2">
						...
						<div class="d-flex">
							...
							<a href="#" class="btn btn-icon btn-active-light-danger w-30px h-30px me-3" onclick="removing('product',<?=$v['id']?>)"><i class="fa fa-trash"></i></a>
						</div>
					</div>
				</div>
					...
					<div class="d-flex flex-wrap py-5">
							...
							<div class="flex-equal me-5">
								<table class="table table-flush fw-bold gy-1">
									<thead>
										<tr><th></th><th><?=$v['var1']?></th><th><?=$v['var2']?></th><th>SKU</th><th>Stok</th><th class="text-right">Harga Dasar</th><?php foreach($atype as $kkkk=>$vvvv){echo'<th class="text-right">Harga '.$vvvv['name'].'</th>';}?></tr>
									</thead>
									<tbody>
										<?php foreach($db->table('product_variant')->where(['product'=>$v['id']])->orderBy('var1 ASC,var2 ASC')->get()->getResultArray() as $kk => $vv){ 
											...
											<tr>
												...
												<td class="text-muted">'.$vv['var1'].'</td>
												<td class="text-muted">'.$vv['var2'].'</td>
												<td class="text-muted">'.$vv['sku'].'</td>
												<td class="text-gray-800 text-right">'.$vv['stock'].'</td>
												<td class="text-gray-800 text-right uang">'.$vv['base_price'].'</td>
												';
												foreach($atype as $kkkk=>$vvvv){
													...
													echo '<td class="text-gray-800 text-right uang">'.$price.'</td>';
												}
												echo '
											</tr>';
										} ?> 
