<?php $db = db_connect();
	...
	if ($id!='') {
		$type = $db->table('account_store_table')->where(['id'=>$id])->get()->getRowArray();
	}
?>
<div class="card mb-5 mb-xl-10" id="thecontent">
	<div class="card-header border-0 cursor-pointer" role="button" data-bs-toggle="collapse" data-bs-target="#kt_account_profile_details" aria-expanded="true" aria-controls="kt_account_profile_details">
		<div class="card-title m-0">
			<h3 class="fw-bolder m-0">Pengaturan Meja <?=$id?></h3>
		</div>
	</div>
					...
					<div class="col-lg-6 col-xs-12"> 
						...
						<div class="fv-row mb-8"> 
							<label class="d-flex align-items-center fs-6 fw-bold form-label mb-2">
								<span class="required">Meja</span>
								<i class="fas fa-exclamation-circle ms-2 fs-7" data-bs-toggle="tooltip" title="Jenis Konsumen"></i>
							</label> 
							<input type="text" class="form-control form-control-solid" placeholder="Meja" name="saver[name]" value="<?=isset($type['name'])?$type['name']:''?>" />
						</div> 
						<div class="fv-row mb-8"> 
							<label class="d-flex align-items-center fs-6 fw-bold form-label mb-2">
								<span class="required">Deskripsi Meja</span>
								<i class="fas fa-exclamation-circle ms-2 fs-7" data-bs-toggle="tooltip" title="Jenis Konsumen"></i>
							</label> 
			...
			<div class="card-footer d-flex justify-content-end py-6 px-9">
				...
				<script>
					...
					function simpan(){
						    ...
						    $.ajax({
						        url: "<?=site_url('pengaturan/meja')?>",  // atau route CI4 kamu
						        type: "POST",
						        data: formData,
