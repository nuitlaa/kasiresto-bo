<?php $db = db_connect();
    ...
    $whr['a.status'] 		= 'active';
    $data = $db->table('account_type a')->join('account_store b','b.id=a.store')->select('a.*,b.name namatoko')->where($whr)->get()->getResultArray();
?>
...
<div class="card pt-4 mb-6 mb-xl-9"> 
	<div class="card-header border-0">
		...
		<div class="card-title">
			<h2 class="fw-bolder mb-0">Jenis Konsumen</h2>
		</div> 
		...
		<div class="card-toolbar">
			...
			<a href="<?=site_url('pengaturan/jenis_konsumen/tambah')?>" class="btn btn-sm btn-flex btn-light-primary">
	...
	<div id="kt_customer_view_payment_method" class="card-body pt-0">
		<table class="table">
			...
			<tr><th>Jenis Konsumen</th><th>Toko</th><th>Aksi</th></tr>
			<?php 
			$star = icon('star');
			$edit = icon('pencil2');
			$more = icon('more'); 
			...
			foreach($data as $K=>$v){  
				?>
					<tr id="row<?=$v['id']?>">
						<td>
							<div class="pe-5">
								<div class="d-flex align-items-center flex-wrap gap-1">
									...
									<div class="badge badge-light-<?=$v['default']==1?'success':'danger'?> ms-5"><?=$v['default']==1?"default":""?></div>
								</div>
							</div>
						</td>
						...
						<td style="text-align: center;"> 
							<a href="<?=site_url('pengaturan/jenis_konsumen/'.$v['id'])?>" class="btn btn-icon btn-active-light-primary w-30px h-30px me-3" ><?=$edit?></a>
