...
class Product_ extends BaseController{
    ...
    public function setminstock(){
        $db = db_connect();
        $variant    = $_POST['variant'];
        $minstock   = $_POST['minstock'];
        $c = $db->table('product_variant')->where(['id'=>$variant])->set(['minstock'=>$minstock])->update();
        $ret['status']      = true;
        $ret['message']     = 'Stok Minimal sudah di rubah';
        echo json_encode($ret);
    }
